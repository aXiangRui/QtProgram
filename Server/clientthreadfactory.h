...
/**
 * @brief 客户端线程工厂类：统一创建三种协议的线程任务
 * @note 职责：根据协议类型和连接参数，创建对应的线程任务（TcpFileTask/HttpTask/WebSocketTask）
...
public:
    ...
    /**
     * @brief 创建线程任务
     * @param type 协议类型（TCP/HTTP/WebSocket）
     * @param socketDescriptor TCP/HTTP连接的套接字描述符（WebSocket无需此参数）
     * @param webSocket WebSocket连接对象（仅WebSocket协议需传入）
     * @return 对应的线程任务（Task子类实例），失败返回nullptr
     */
    Task* createThread(ProtocolType type, qintptr socketDescriptor = 0, QWebSocket* webSocket = nullptr);
